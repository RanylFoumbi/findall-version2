...
/**
 * isMine : to know if this object has been published by the current user;
 * hasBeenFound: to know if the object has already been found.
 */
  final int index;
...
class _DetailLostPageState extends State<DetailLostPage> {
  ...
  void initState() {
    ...
    super.initState();
       WidgetsBinding.instance.addPostFrameCallback((_) async {
         seen.ready.then((_) => {
         if( checkIfFirstSeen(widget.index) == false && widget.isMine == true) {
           _boostSearchDialog()
         }else{
  ...
  _thisObjectHasBeenFound(){
    print('it has been found');
    setState(() {
      widget.hasBeenFound = !widget.hasBeenFound;
    });
    db.collection('ObjectList').document(widget.docID).updateData({'hasBeenFound': widget.hasBeenFound});
  }
          ...
          builder: (BuildContext context){
            return SimpleDialog(
              ...
              title: Row(
                ...
                children: <Widget>[
                  Expanded(
                    child: Text("Booster vos chances de touver votre "+' '+ widget.objectName,style: TextStyle(fontWeight: FontWeight.w500,fontFamily: "Raleway"),
                  ...
                  children: <Widget>[
                    ...
                    Container(
                      ...
                      child: FloatingActionButton.extended(
                        ...
                        label: Text('Booster Maintenant',style: TextStyle(fontSize: 16
  ...
  Widget build(BuildContext context) {
                                                  ...
                                                  child: ClipRRect(
                                                    ...
                                                    child: CachedNetworkImage(
                                                        ...
                                                        placeholder: (context, url) =>
                                                            SpinKitFadingCircle(color: Colors.deepPurple, size: 17),
                                                        errorWidget: (context, url, error) =>
                                                        new Icon(Icons.error, color: Colors.deepPurple, size: 17)
                                                    )
                                                  ),
                                                  onTap: () {
    ...
    final reward = Container(
        child: Row(
          ...
          mainAxisAlignment: MainAxisAlignment.start,
          children: <Widget>[
            Text('Motivation:', style: TextStyle(color: Colors.grey,
                fontWeight: FontWeight.bold,
                fontSize: 12,
                fontFamily: 'Raleway')),
            SizedBox(width: width / 30),
    ...
    final postownerName = Container(
        child: Row(
          ...
          children: <Widget>[
            ...
            SizedBox(width: 20),
            Expanded(child: Text(widget.ownerName == '' || widget.ownerName.runtimeType == Null ? "Anonyme" : widget.ownerName, style: TextStyle(
                color: Colors.black.withOpacity(0.6),
                fontWeight: FontWeight.bold,
                fontSize: 13,
                fontFamily: 'Raleway')))
    ...
    final bottomButton = Row(
      ...
      mainAxisAlignment: widget.isMine ? MainAxisAlignment.spaceAround : MainAxisAlignment.center,
      children: <Widget>[

        Container(
          height: 50,
          width: widget.isMine ? width/2.6 : width/1.2,
          child: widget.isMine
                              ?
                                FloatingActionButton.extended(
                                  icon: Icon(widget.hasBeenFound ? Icons.check_box : Icons.check_box_outline_blank,size: 28,color: Colors.white),
                                  label: Text('Trouvé',style: TextStyle(fontFamily: "Raleway"),),
                                  shape: RoundedRectangleBorder(
                                      borderRadius: BorderRadius.circular(10)
                                  ),
                                  backgroundColor: Colors.pink,
                                  heroTag: "found"+ widget.index.toString(),
                                  onPressed: (){
                                    _thisObjectHasBeenFound();
                                  },
                                )
                              :
                                FloatingActionButton.extended(
                                  ...
                                  label: Text('Appeller',style: TextStyle(fontFamily: "Raleway"),),
                                  shape: RoundedRectangleBorder(
                                      borderRadius: BorderRadius.circular(10)
                                  ),
                                  backgroundColor: Colors.pink,
